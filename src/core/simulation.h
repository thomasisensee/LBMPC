...
protected:
    ...
    std::unique_ptr<LBGrid<T,D,Q>> _lbGrid;
...
public:
    /// Constructor
    Simulation(std::unique_ptr<LBGrid<T,D,Q>>&& lbgrid, std::unique_ptr<VTKWriter>&& vtkWriter, T dt, T simTime, unsigned int numberOutput);
...
/***************************/
template<typename T, unsigned int D, unsigned int Q>
class LBFluidSimulation final : public Simulation<T,D,Q> {
public:
    /// Constructor
    LBFluidSimulation(std::unique_ptr<LBGrid<T,D,Q>>&& lbgrid, std::unique_ptr<VTKWriter>&& vtkWriter, T dt, T simTime, unsigned int numberOutput);
